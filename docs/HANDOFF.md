...
**Branch:** `main` (ahead of `v0.2.0` tag by 8 commits)
**Working tree:** clean (todo + handoff refresh pending)
...
- **Trivia filter + attribution stripper are in place.** `TranscriptFilter` drops `*Bip*` / `[Music]` / `thank you`-style chunks before the LLM. `AttributionStripper` removes leading "The speaker said…" / "User claims…" from any bullets that do slip through. Both have broad unit coverage (39/39 tests green on main).
...
### Today (2026-04-20) — acted on `m13v`'s external feedback on #7 and #8

Research doc: [`docs/superpowers/research-2026-04-19-m13v-feedback.md`](./superpowers/research-2026-04-19-m13v-feedback.md).

- **PR #16 (issue-8-vad-grace-period)** — supersedes PR #10. Replaces wall-clock `trailingSilenceMs` with `endOfUtteranceGraceMs` (default 1200 ms). Sub-grace breaths concatenate instead of splitting paragraphs for rapid speakers. 7 new tests bring suite to **46/46 green on the PR branch** (still 39 on main until merge). See PR body for the regression test calibrated between PR #10's 900 ms and the new 1200 ms.
- **PR #17 (issue-7-speech-daemon-exclusion)** — supersedes PR #14. Adds `SCContentFilter.excludingApplications` against six macOS speech daemons (TTS / dictation / CoreSpeech / Siri) so recognition chirps and TTS playback stop bleeding into the SCK mix. Also switches `SCShareableContent.excludingDesktopWindows(_:onScreenWindowsOnly:)` from `true` → `false` so the background daemons are discoverable. No new unit tests — SCK is integration-only, live verification required.
- **Explicitly deferred**: Silero v5 VAD (research verdict: not a fix for the reported bug), `AudioHardwareCreateProcessTap` migration (bigger rewrite; on the long-term list), transformer EoU (overkill).
...
| #13 | `issue-6-ollama-model-picker`             | New `OllamaModelsProbe` + picker in Settings → Backend. Needs UI smoke-test.                              |
| #15 | `issue-5-mode-change-log`                 | Diagnostic / instrumentation only. Real fix pending a live repro with `/tmp/notchy-debug.log`.            |
| **#16** | **`issue-8-vad-grace-period`**        | **Grace-period VAD (supersedes #10).** 46/46 tests green on branch. Needs live rapid-speaker A/B.         |
| **#17** | **`issue-7-speech-daemon-exclusion`** | **SCK exclude speech daemons (supersedes #14).** Needs live dictation-chirp test.                         |

Superseded / to close: **#10** (bandaid, replaced by #16), **#14** (research-only, replaced by #17).
...
- **Dictation cross-talk (residual, after PR #17 merges):** the app-level SCK exclusion kills leakage from Apple's speech daemons, but user-side audio routing still leaks — Live Listen, conferencing apps with monitor-my-mic, aggregate devices (BlackHole / Loopback) that send mic → default output. Those are out of scope for SCK filtering. Long-term fix is migrating to `AudioHardwareCreateProcessTap` (macOS 14.4+).
